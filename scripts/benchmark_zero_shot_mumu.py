...
OUT_CSV = os.path.join(PROJ_DIR, "outputs", "zero_shot_mumu.csv")
PYTHON_BIN = "/data/mg546924/conda_envs/qwenenv/bin/python"
...
def main():
    ...
    print(f"Found {len(songs)} songs. Benchmarking MuMu...")
    ...
    with open(OUT_CSV, "w", newline="") as f:
        ...
        for song_name, audio_path in songs:
            print(f"\n-> Querying MuMu for {song_name}...")
            pred_bpm = run_model_query(audio_path)
            print(f"   BPM: {pred_bpm}")
            writer.writerow([song_name, "MuMu", pred_bpm])
            f.flush()
