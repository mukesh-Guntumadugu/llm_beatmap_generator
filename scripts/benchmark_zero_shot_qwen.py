...
OUT_CSV = os.path.join(PROJ_DIR, "outputs", "zero_shot_qwen.csv")
...
def main():
    ...
    print(f"Found {len(songs)} songs. Benchmarking Qwen...")
    ...
    with open(OUT_CSV, "w", newline="") as f:
        ...
        for song_name, audio_path in songs:
            print(f"\n-> Querying Qwen for {song_name}...")
            pred_bpm = run_model_query(audio_path)
            print(f"   BPM: {pred_bpm}")
            writer.writerow([song_name, "Qwen", pred_bpm])
            f.flush()
