#!/usr/bin/env python3
"""MuMu-LLaMA onset test on Bad Ketchup."""
...
out = os.path.join(OUT_DIR, f"MuMuChunked_TEST_Bad_Ketchup_{ts}.csv")
with open(out, "w", newline="") as f:
    w = csv.writer(f); w.writerow(["onset_ms"])
    for ms in all_onsets: w.writerow([ms])

print(f"✅ MuMu-LLaMA found {len(all_onsets)} onsets", flush=True)
