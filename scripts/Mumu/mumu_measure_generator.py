...
try:
    from src.mumu_measure_interface import initialize_mumu_model, get_mumu_16_step_probabilities
except ImportError:
    print("Could not import src.mumu_measure_interface. Ensure BENCHMARK_PROJ is correct.")
    sys.exit(1)
...
def main():
    print(f"Loading MuMu-LLaMA model...")
    ...
    # Output Subfolder Logic specific to MuMu
    global OUT_DIR
    OUT_DIR = os.path.join(OUT_DIR, "MuMu", "Wonsets_Wtempo_WBPM")
    ...
    for M in tqdm(range(total_measures), desc="Generating Beats (Beat-by-Beat)"):
        ...
        for b in range(4):
            ...
            if has_onset:
                prompt += f"CONDITION: ONSET DETECTED. You must select a valid 4-character step (e.g. 0001, 0110). Do NOT output 0000 unless absolutely necessary.\n"
            else:
            ...
            # HARD BYPASS: Skip model inference entirely if no onset
            if not has_onset:
                selected_step = "0000"
                ...
                # Score true mathematical probabilities for the 16 paths using MuMu-LLaMA
                probs_dict = get_mumu_16_step_probabilities(
    ...
    csv_path = os.path.join(OUT_DIR, f"MuMu_MeasureGrid_Wonsets_Wtempo_WBPM_{ts}.csv")
    txt_path = os.path.join(OUT_DIR, f"MuMu_MeasureGrid_Wonsets_Wtempo_WBPM_{ts}.txt")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        w.writerow(["# Graph Search Strategy", "Exhaustive Sequence Evaluation over MuMu-LLaMA c_loss"])
        w.writerow(["# Temperature", "1.8"])
        w.writerow(["# Top-P", "0.9"])
        w.writerow(["# Min-P", "0.01"])
        w.writerow(["# Top-K", "None"])
        w.writerow(["# Repetition Penalty", "Dynamic Scale Matrix (1.0 + N*0.5)"])
