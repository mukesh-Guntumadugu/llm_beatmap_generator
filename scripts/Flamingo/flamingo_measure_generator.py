...
try:
    from src.flamingo_measure_interface import initialize_flamingo_model, get_flamingo_16_step_probabilities
except ImportError:
    print("Could not import src.flamingo_measure_interface. Ensure BENCHMARK_PROJ is correct.")
    sys.exit(1)
...
def main():
    print("Loading Music-Flamingo model (~30GB)...")
    ...
    print(f"Total Measures to generate: {total_measures}")
    print("--------------------------------------------------")

    global OUT_DIR
    OUT_DIR = os.path.join(OUT_DIR, "Flamingo", "Wonsets_Wtempo_WBPM")
    ...
    for M in tqdm(range(total_measures), desc="Generating Beats (Beat-by-Beat)"):
        ...
        for b in range(4):
            ...
            if has_onset:
                prompt += f"CONDITION: ONSET DETECTED. Select a valid 4-character step (e.g. 0001, 0110). Do NOT output 0000.\n"
            else:
            ...
            # HARD BYPASS for no-onset beats
            if not has_onset:
                selected_step = "0000"
                ...
                probs_dict = get_flamingo_16_step_probabilities(
    ...
    csv_path = os.path.join(OUT_DIR, f"Flamingo_MeasureGrid_Wonsets_Wtempo_WBPM_{ts}.csv")
    txt_path = os.path.join(OUT_DIR, f"Flamingo_MeasureGrid_Wonsets_Wtempo_WBPM_{ts}.txt")
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        w.writerow(["# Graph Search Strategy", "Exhaustive Sequence Evaluation via AudioFlamingo3 loss"])
